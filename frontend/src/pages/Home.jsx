...
function Home() {
    ...
    const fetchAllTasks = async () => {
        try {
            const res  = await fetch('https://task-manager-nwae.onrender.com/tasks')
    ...
    const fetchTasks = async () => {
        try {
            let url = 'https://task-manager-nwae.onrender.com/tasks'
    ...
    useEffect(() => { fetchTasks(); fetchAllTasks() }, [filter])
    ...
    return (
        <div className="page">
            ...
            <div className="tasks">
                {tasks.length > 0 ? (
                    tasks.map(task => (
                        <TaskCard key={task._id} task={task} onDelete={refresh} />
                    ))
                ) : (
                    <div className="empty">
                        <p className="empty__title">No tasks here</p>
                        <p className="empty__sub">Hit "+ Add Task" to get started</p>
                    </div>
                )}
            </div>
